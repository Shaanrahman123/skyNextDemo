...
const HolidayPackageResultMain = () => {
  ...
  useEffect(() => {
    if (type == "cities") {
      dispatch(searchPackageAction(destination));
    }
    if (type == "category") {
      dispatch(searchPackageActionCategory(destination));
    }
    if (type == "country") {
      //   dispatch(searchPackageActionTopCountries(destination));
    }
    sessionStorage.setItem("searchPackageData", JSON.stringify(destination));
          ...
          {tooManyFilter ? (
            <div className="col-lg-9">
              <div className="noHotels">
                ...
                <p>
                  Please remove some <Pencil width={20} />
                </p>
              </div>
            </div>
          ) : (
